...
ch_analog0   = 0 # analog channel 0
ch_analog1   = 1 # analog channel 1
...
analogRampTime      = 20 #ns
analogRampDelay     = 50 #ns
analogRampDownTime     = 100 #ns
analogRampOnTime    = 50 #ns
...
# make an analog pulse pattern with a ramp for the analog channels turning on and off the analog output
# make a list of tuples with (duration, voltage) for the analog channel 0 ramping up to 1V and down to 0V again. The ramp is done with a linear interpolation of the voltage values between 0 and 1V
ramp_up = [(t, v) for t, v in zip([analogRampTime/11]*11, np.linspace(0, 1, num=11))]
ramp_down = [(t, v) for t, v in zip([analogRampTime/11]*11, np.linspace(1, 0, num=11))]

print("Analog ramp up: ", ramp_up)
print("Analog ramp down: ", ramp_down)

seq_analog0 = [(analogRampDelay, 0)] + ramp_up + [(analogRampOnTime, 1)] + ramp_down
seq_analog1 = [(analogRampOnTime, 1)] + ramp_down + [(analogRampDownTime, 0)] + ramp_up + [(analogRampOnTime, 1)] + ramp_down
...
seq.setAnalog(ch_analog0, seq_analog0)
seq.setAnalog(ch_analog1, seq_analog1)
