...
# standard XY8 refocusing-pulse phase pattern
XY8_PHASES = [PHASE_X, PHASE_Y, PHASE_X, PHASE_Y, PHASE_Y, PHASE_X, PHASE_Y, PHASE_X]
...
class XYSequence(RFSocSequence):
    """RFSoC XY8-N dynamical-decoupling sequence builder.

    pi/2(x) - tau/2 - [pi(phase) - tau]*(8*n_reps-1) - pi(phase) - tau/2 -
    pi/2(x), where the pi pulse phases cycle through the standard XY8
    pattern [x, y, x, y, y, x, y, x] repeated ``n_reps`` times, sweeping the
    total free precession time ``sweep_x`` (interpulse spacing is
    ``sweep_x / (8 * n_reps)``).
    """
    ...
    def extra_log_info(self):
        return (
            ...
            + f"\nNumber of XY8 repeats: {self.n_reps}"
    ...
    def load(
        ...
        **kwargs,
        ...
        if sweep_x is None:
            logger.error("sweep_x (total free precession times) must be provided for the XY8 sequence.")
            return
        if n_reps < 1:
            logger.error("n_reps must be at least 1 for the XY8 sequence.")
            return

        self.frequency = frequency
        self.n_reps = int(n_reps)
        phases = XY8_PHASES * self.n_reps
        self.n_pulses = len(phases)
